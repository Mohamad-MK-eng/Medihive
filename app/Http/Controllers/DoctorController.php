...
class DoctorController extends Controller
{
    ...
    public function getTimeSlots(Request $request): mixed
    {
        $doctor = Auth::user()->doctor;
    ...
    public function getPatientDocuments(Request $request, $patientId)
{
    try {
        ...
        $reportsQuery = Report::select('reports.id', 'reports.title', 'appointments.appointment_date')
            ->join('appointments', 'appointments.id', '=', 'reports.appointment_id')
            ->where('appointments.doctor_id', $doctor->id)
            ->where('appointments.patient_id', $patientId);

        if ($request->has('year')) {
            $reportsQuery->whereYear('appointments.appointment_date', $year);
        }

        if ($request->has('month')) {
            $reportsQuery->whereMonth('appointments.appointment_date', $month);
        }

        $reports = $reportsQuery->orderBy('appointments.appointment_date', 'desc')
            ->get();
