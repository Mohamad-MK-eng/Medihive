...
public function deleteClinic($clinic_id)
{
    ...
    $hasDoctors = $clinic->doctors()->exists();
...
public function addPatient(Request $request)
{
    ...
    try {
        ...
        $user = User::create([
            ...
            'gender' => $request->gender,
            'role_id' => 4, // <-- هام: تأكد من أن هذا هو الـ ID الصحيح لدور "Patient" في جدول roles
        ]);
...
public function deleteSecretary($id)
    {
        ...
        if ($secretary->user->profile_picture) {

            $picturePath = str_replace('/storage/', '', $secretary->user->profile_picture);
        ...
        if (Secretary::find($id)) {
            $secretary->delete();
        }

        return response()->json(['success' => true, 'message' => 'deleted suceccfully']);
    }
...
public function updateSecretary(Request $request, $id)
{
    ...
    // Use 'sometimes' validation instead of 'nullable'
    $validated = $request->validate([
        'first_name'       => 'sometimes|string|max:255',
        'last_name'        => 'sometimes|string|max:255',
        'email'            => [
            'sometimes',
            'email',
            'unique:users,email,' . $secretary->user->id
        ],
        'workdays'         => 'sometimes|string',
        'profile_picture'  => 'sometimes|image|mimes:jpeg,png,jpg,gif|max:4096'
    ]);

    // Update user table only if fields are provided
    if (isset($validated['first_name'])) {
        $secretary->user->first_name = $validated['first_name'];
    }

    if (isset($validated['last_name'])) {
        $secretary->user->last_name = $validated['last_name'];
    }
